...
async function startAgent(options = {}) {
  ...
  if (options.autoPublish !== undefined)
    env.AUTO_PUBLISH = String(options.autoPublish);
  if (options.liveCrawl !== undefined)
    env.USE_LIVE_CRAWL = String(options.liveCrawl);
...
async function findAvailablePort() {
  ...
  if (envPort) {
    ...
    console.log(
      `Warning: Port ${port} is in use, searching for alternative...`,
    );
  }
