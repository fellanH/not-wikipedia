...
class LoopCanvas {
    ...
    drawNode(phase, isActive, isCurrent) {
        ...
        // Label
        ctx.fillStyle = isActive && isCurrent ? '#fff' : (isActive ? phase.color : INACTIVE_COLOR);
    ...
    drawCenter(isActive, activeWorkers) {
        ...
        ctx.fillText(activeWorkers + ' worker' + (activeWorkers !== 1 ? 's' : ''), this.centerX, this.centerY + 10);
    }
    ...
    render(workers, isActive) {
        ...
        const activeWorkerCount = workers.filter(w => w.phase && w.phase !== 'idle').length;
...
function getWorkerColor(id) {
    const colors = ['#00bcd4', '#ff9800', '#9c27b0', '#4caf50', '#2196f3', '#f44336'];
...
function updateUI(data) {
    ...
    const activeWorkers = (data.workers || []).filter(w => w.phase && w.phase !== 'idle').length;
    document.getElementById('active-workers').textContent = activeWorkers + '/' + (data.workers || []).length;

    document.getElementById('recent-count').textContent = (data.recentArticles || []).length;
...
function updateWorkersGrid(workers) {
    ...
    grid.innerHTML = workers.map(worker => {
        ...
        const taskInfo = worker.task ? worker.task.filename || worker.task.type : '';
...
function updateTaskQueue(queue) {
    ...
    document.getElementById('queue-broken').style.width = Math.min(broken / maxQueue * 100, 100) + '%';
    document.getElementById('count-broken').textContent = broken;

    document.getElementById('queue-orphan').style.width = Math.min(orphan / maxQueue * 100, 100) + '%';
    document.getElementById('count-orphan').textContent = orphan;

    document.getElementById('queue-discovery').style.width = Math.min(discovery / maxQueue * 100, 100) + '%';
...
function updateRecentArticles(articles) {
    ...
    container.innerHTML = articles.slice(0, 10).map((article, idx) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    poller.register('status', CONFIG.statusFile, CONFIG.pollInterval, (data, prev, error) => {
        if (error) {
            handleError(error);
            return;
        }
