...
const TOOL_DEFINITIONS = {
    ...
    'wiki-git-publish': { args: [{ name: 'message', type: 'text', label: 'Commit message (optional)' }] },
    'wiki-get-article': { args: [{ name: 'path', type: 'text', label: 'Article path (e.g., semantic_debt)' }] },
    'wiki-create-article': {
        args: [
            { name: 'path', type: 'text', label: 'Article path' },
            { name: 'title', type: 'text', label: 'Title' },
            { name: 'type', type: 'text', label: 'Type (concept/researcher/institution)' },
...
function updateToolForm() {
    ...
    if (def && def.args.length > 0) {
        ...
        argsContainer.innerHTML = '<p style="color: var(--text-muted); font-size: 0.85rem;">No arguments required</p>';
    }
}
