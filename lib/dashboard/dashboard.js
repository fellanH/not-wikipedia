...
// API base URL (when using server.js)
const API_BASE = window.location.origin;
let isServerMode = false;
let statusPollInterval = null;

// Get the wiki content path (different in server vs static mode)
function getWikiPath() {
  return isServerMode ? "/wiki" : "../not-wikipedia";
}
...
function updateEcosystemMetrics(data) {
  ...
  brokenEl.className =
    "value " + (stats.broken_links > 0 ? "error" : "success");

  const orphanEl = document.getElementById("orphan-count");
  orphanEl.textContent = stats.orphan_articles;
  orphanEl.className =
    "value " + (stats.orphan_articles > 0 ? "warning" : "success");

  document.getElementById("avg-links").textContent =
    stats.avg_links_per_article;
}
...
function updateResearcherMetrics(data) {
  ...
  const overused = researchers.filter(
    (r) => r.status && r.status.includes("OVERUSED"),
  ).length;
  const moderate = researchers.filter(
    (r) => r.status && r.status.includes("MODERATE"),
  ).length;
  ...
  try {
    ...
    for (let loop = 100; loop >= 1; loop--) {
      try {
        ...
        const testUrls = [`../.logs/run-${dateStr}-`];
...
async function buildAndRenderGraph(ecosystem) {
  ...
  const linkPromises = articleIds.map(async (id) => {
    try {
      const response = await fetch(`${getWikiPath()}/${id}.html`);
  ...
  network.on("doubleClick", function (params) {
    if (params.nodes.length > 0) {
      const nodeId = params.nodes[0];
      window.open(`${getWikiPath()}/${nodeId}.html`, "_blank");
    }
  });
}
...
async function discoverLogFiles() {
  ...
  if (logFiles.length > 0) {
    ...
    document.getElementById("last-run").textContent = formatLogTime(
      latest.date,
      latest.time,
    );
  }
...
function populateLogSelector() {
  ...
  for (const log of logFiles.slice(0, 50)) {
    // Show last 50 logs
...
function renderRecentRuns() {
  ...
  for (const log of logFiles.slice(0, 12)) {
    ...
    card.innerHTML = `
            ...
            <div class="run-stats">
              <span class="run-error-badge" data-path="${log.path}"></span>
            </div>
        `;
  ...
  // Scan recent logs for error counts (async, updates badges as they load)
  scanRunsForErrors(logFiles.slice(0, 12));
}

// Scan log files and populate error badges on run cards
async function scanRunsForErrors(logs) {
  for (const log of logs) {
    try {
      const response = await fetch(log.path);
      if (!response.ok) continue;
      const text = await response.text();
      const events = parseNDJSON(text);
      const data = processLogEvents(events);
      const errorCount = data.errors.length;

      const badge = document.querySelector(
        `.run-error-badge[data-path="${log.path}"]`,
      );
      if (badge) {
        if (errorCount > 0) {
...
function processLogEvents(events) {
  ...
  for (const event of events) {
              ...
              if (toolCall) {
                ...
                if (block.is_error) {
                  result.errors.push({
                    ...
                    timestamp: event.timestamp || event.uuid,
                  });
                }
              }
            }
    ...
    // Capture stderr lines as errors
    if (event.subtype === "stderr" || event.type === "stderr") {
      const text = event.text || event.message || event.data || "stderr output";
      result.errors.push({
        type: "stderr",
...
function updateLogSummary(data) {
  ...
  document.getElementById("log-session").textContent = data.sessionId
    ? data.sessionId.slice(0, 8) + "..."
    : "--";
  document.getElementById("log-model").textContent = data.model
    ? data.model.replace("claude-", "").replace("-20251101", "")
    : "--";
  document.getElementById("log-tool-count").textContent = data.toolCalls.length;
  document.getElementById("log-message-count").textContent =
  ...
  if (errorCountEl) {
    ...
    errorCountEl.className =
      "log-stat-value" + (errorCount > 0 ? " error" : " success");
  }
}
...
function renderActivityTimeline(data) {
  ...
  // Sort by timestamp (prefer ISO timestamp, fall back to UUID)
  timeline.sort((a, b) => (a.order || "").localeCompare(b.order || ""));

  if (timeline.length === 0) {
    container.innerHTML =
      '<p class="empty-state">No activity recorded in this log</p>';
    return;
  }

  // Error summary banner at top of timeline
  if (data.errors.length > 0) {
    const banner = document.createElement("div");
    banner.className = "error-summary-banner";
    const errorTypes = {};
    for (const err of data.errors) {
      errorTypes[err.type] = (errorTypes[err.type] || 0) + 1;
    }
    const breakdown = Object.entries(errorTypes)
      .map(([type, count]) => `${count} ${type.replace("_", " ")}`)
      .join(", ");
    banner.innerHTML = `
      <span class="error-summary-count">${data.errors.length} error${data.errors.length !== 1 ? "s" : ""}</span>
      <span class="error-summary-detail">${breakdown}</span>
    `;
...
function createTimelineEntry(entry) {
  ...
  if (entry.type === "message") {
    ...
    const err = entry.data;
    const typeLabel =
      err.type === "tool_error"
        ? `Tool Error (${err.tool || "unknown"})`
        : err.type === "stderr"
          ? "stderr"
          : "Error";
    div.innerHTML = `
            ...
            <div class="timeline-content">
                <div class="timeline-header">
                    <span class="timeline-type timeline-type-error">${typeLabel}</span>
                </div>
                <div class="timeline-message stderr">${escapeHtml(typeof err.message === "string" ? err.message.slice(0, 500) : JSON.stringify(err.message).slice(0, 500))}</div>
            </div>
        `;
  }
...
async function analyzeWordUsage() {
  ...
  try {
    ...
    const results = (await Promise.all(analysisPromises)).filter(
      (r) => r !== null,
    );
...
function extractTextFromHtml(html) {
  ...
  temp
    .querySelectorAll("script, style, .toc, .ambox, .infobox, .reflist")
...
function renderWordStats(stats) {
  ...
  document.getElementById("avg-words-article").textContent =
    stats.avgWordsPerArticle.toLocaleString();
  document.getElementById("vocabulary-richness").textContent =
  ...
  container.innerHTML = shuffled
    .map(([word, count]) => {
      ...
      if (
        ...
        word.includes("memory") ||
        word.includes("psych")
      ) {
        catClass = "cat-consciousness";
      } else if (
        word.includes("tempor") ||
        word.includes("chrono") ||
        word.includes("time")
      ) {
        catClass = "cat-chronopsychology";
      } else if (
        word.includes("digit") ||
        word.includes("algorithm") ||
        word.includes("comput")
      ) {
...
function filterWordsByCategory() {
  ...
  if (currentCategoryFilter === "all") {
    ...
    renderWordList(
      "top-words",
      wordStatsData.topWords,
      wordStatsData.topWords[0]?.[1] || 1,
    );
    renderWordList(
      "domain-terms",
      wordStatsData.domainTerms,
      wordStatsData.domainTerms[0]?.[1] || 1,
    );
  } else {
...
// ============================================
// SERVER MODE & AGENT CONTROL
// ============================================

// Check if running via server.js (has API endpoints)
async function checkServerMode() {
  try {
    const response = await fetch(`${API_BASE}/api/status`, { method: "GET" });
    if (response.ok) {
      isServerMode = true;
      console.log("Running in server mode with API support");
    ...
    isServerMode = false;
    console.log("Running in static mode (no API support)");
    // Hide control panel elements that require server
    const controlPanel = document.querySelector(".control-panel");
    if (controlPanel) {
      controlPanel.innerHTML = `
                <div class="card control-card" style="grid-column: span 3;">
                    <h3>Server Mode Required</h3>
                    <p style="color: var(--text-muted); padding: 1rem;">
                        To enable agent controls and tool testing, start the dashboard with:
                        <code style="background: #f0f0f0; padding: 0.25rem 0.5rem; border-radius: 4px; margin: 0 0.25rem;">npm run dashboard</code>
                        <br><br>
                        Currently viewing in static mode (read-only).
                    </p>
                </div>
            `;
    }
...
function startStatusPolling() {
  ...
  statusPollInterval = setInterval(updateAgentControlStatus, 2000);
}

// Update agent control panel status
async function updateAgentControlStatus() {
  ...
  try {
    ...
    const statusText = document.getElementById("agent-running-status");
...
// Render mini logs in control panel
function renderMiniLogs(logs) {
  const container = document.getElementById("agent-logs-mini");
  if (!container) return;
  ...
  container.innerHTML = logs
    ...
    .map((log) => {
      ...
      return `<div class="log-line ${log.type}">[${time}] ${escapeHtml(log.text)}</div>`;
    })
    .join("");
...
async function startAgentFromUI() {
  if (!isServerMode) {
    alert("Server mode required. Run: npm run dashboard");
    return;
  }

  const workers = parseInt(document.getElementById("opt-workers").value) || 3;
  const loops = parseInt(document.getElementById("opt-loops").value) || 100;
  const autoPublish = document.getElementById("opt-publish").checked;

  try {
    const response = await fetch(`${API_BASE}/api/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ workers, loops, autoPublish }),
    });
...
// Stop agent from UI
async function stopAgentFromUI() {
  if (!isServerMode) return;
...
// Tool definitions with expected arguments
const TOOL_DEFINITIONS = {
  "wiki-ecosystem": { args: [] },
  "wiki-next-task": { args: [] },
  "wiki-build-index": { args: [] },
  "wiki-discover": { args: [] },
  "wiki-human-seed": { args: [] },
  "wiki-crawl-404s": { args: [] },
  "wiki-git-publish": {
    args: [
      { name: "message", type: "text", label: "Commit message (optional)" },
    ],
  },
  "wiki-get-article": {
    args: [
      {
        name: "path",
        type: "text",
        label: "Article path (e.g., semantic_debt)",
      },
    ],
  },
  "wiki-create-article": {
    args: [
      { name: "path", type: "text", label: "Article path" },
      { name: "title", type: "text", label: "Title" },
      {
        name: "type",
        type: "text",
        label: "Type (concept/researcher/institution)",
      },
      { name: "category", type: "text", label: "Category" },
      { name: "summary", type: "textarea", label: "Summary" },
      { name: "content", type: "textarea", label: "Content HTML" },
    ],
  },
  "wiki-edit-article": {
    args: [
      { name: "path", type: "text", label: "Article path" },
      { name: "section", type: "text", label: "Section to edit" },
      { name: "content", type: "textarea", label: "New content" },
    ],
  },
  "wiki-add-link": {
    args: [
      { name: "sourcePath", type: "text", label: "Source article path" },
      { name: "targetPath", type: "text", label: "Target article path" },
...
async function loadToolList() {
  ...
  try {
    ...
    if (data.success && data.tools) {
      ...
      for (const tool of data.tools.sort()) {
        const option = document.createElement("option");
        option.value = tool;
...
// Update tool form based on selection
function updateToolForm() {
  const select = document.getElementById("tool-select");
  const form = document.getElementById("tool-form");
  const argsContainer = document.getElementById("tool-args");
  ...
  if (def && def.args.length > 0) {
    for (const arg of def.args) {
      ...
      if (arg.type === "textarea") {
        ...
        textarea.id = `arg-${arg.name}`;
        textarea.name = arg.name;
        argsContainer.appendChild(textarea);
      } else {
    ...
    argsContainer.innerHTML =
      '<p style="color: var(--text-muted); font-size: 0.85rem;">No arguments required</p>';
  }
}
  ...
  if (def && def.args) {
    for (const arg of def.args) {
      ...
      if (input && input.value.trim()) {
        args[arg.name] = input.value.trim();
      }
    }
  }
...
// Run quick action
async function runQuickAction(toolName) {
  if (!isServerMode) {
    alert("Server mode required. Run: npm run dashboard");
    return;
  }
...
// Open local article preview
function openLocalPreview() {
  const articleId = prompt("Enter article ID (e.g., semantic_debt):");
  if (!articleId) return;

  // Open the article in a new tab using the local file path
  window.open(`${getWikiPath()}/${articleId}.html`, "_blank");
}
...
async function fetchSeed() {
  ...
  const btn = document.getElementById("btn-fetch-seed");
  btn.disabled = true;
  btn.textContent = "Fetching...";

  try {
    const fallback = document.getElementById("seed-fallback").checked;
    const resp = await fetch(
      `${API_BASE}/api/seed${fallback ? "?fallback=true" : ""}`,
    );
    const data = await resp.json();
    const seed = parseSeedToolResult(data);

    const display = document.getElementById("seed-display");
    display.classList.remove("hidden");

    // Hide voice/hint/task panels (seed-only mode)
    document.getElementById("seed-voice").classList.add("hidden");
    document.getElementById("seed-hint").classList.add("hidden");
    document.getElementById("seed-task").classList.add("hidden");

    if (seed) {
      document.getElementById("seed-text").textContent =
        seed.text || seed.raw || JSON.stringify(seed);
      document.getElementById("seed-source").textContent = seed.source || "";
      const badge = document.getElementById("seed-type");
      badge.textContent = seed.type || "unknown";
      badge.className = `seed-type-badge seed-type-${seed.type || "unknown"}`;
    } else {
      document.getElementById("seed-text").textContent =
        "No seed data returned";
      document.getElementById("seed-source").textContent = "";
      document.getElementById("seed-type").textContent = "";
    }
  } catch (e) {
    alert("Error fetching seed: " + e.message);
  } finally {
    btn.disabled = false;
    btn.textContent = "Fetch Seed";
  }
}

async function fetchSeedPreview() {
  if (!isServerMode) {
    alert("Server mode required. Run: npm run dashboard");
    return;
  }

  const btn = document.getElementById("btn-seed-preview");
  btn.disabled = true;
  btn.textContent = "Loading...";

  try {
    const resp = await fetch(`${API_BASE}/api/seed/preview`);
    const data = await resp.json();
    const task = parseSeedToolResult(data);
    ...
    // Human seed
    if (task.humanSeed) {
      document.getElementById("seed-text").textContent = task.humanSeed.text;
      document.getElementById("seed-source").textContent =
        task.humanSeed.source || "";
      const badge = document.getElementById("seed-type");
      badge.textContent = task.humanSeed.type || "unknown";
      badge.className = `seed-type-badge seed-type-${task.humanSeed.type || "unknown"}`;
    } else {
      document.getElementById("seed-text").textContent =
        "(No human seed — task is a repair/fix)";
      document.getElementById("seed-source").textContent = "";
      document.getElementById("seed-type").textContent = task.taskType;
      document.getElementById("seed-type").className =
        "seed-type-badge seed-type-task";
    }

    // Voice context
    const voicePanel = document.getElementById("seed-voice");
    if (task.voiceContext) {
      voicePanel.classList.remove("hidden");
      document.getElementById("voice-text").textContent =
        task.voiceContext.text;
      document.getElementById("voice-source").textContent =
        task.voiceContext.source || "";
      const vBadge = document.getElementById("voice-type");
      vBadge.textContent = task.voiceContext.type || "unknown";
      vBadge.className = `seed-type-badge seed-type-${task.voiceContext.type || "unknown"}`;
    } else {
      voicePanel.classList.add("hidden");
    }

    // Subject hint
    const hintPanel = document.getElementById("seed-hint");
    if (task.subjectHint) {
      hintPanel.classList.remove("hidden");
      document.getElementById("hint-value").textContent = task.subjectHint;
    } else {
      hintPanel.classList.add("hidden");
    }

    // Task context
    const taskPanel = document.getElementById("seed-task");
    taskPanel.classList.remove("hidden");
    const meta = document.getElementById("task-meta");
    meta.innerHTML = `
      <div class="task-meta-row"><strong>Task Type:</strong> <span class="task-type-badge task-type-${task.taskType}">${task.taskType}</span></div>
      <div class="task-meta-row"><strong>Priority:</strong> <span class="priority-badge priority-${task.priority}">${task.priority}</span></div>
      <div class="task-meta-row"><strong>Topic:</strong> ${escapeHtml(task.topic?.name || "—")}</div>
      <div class="task-meta-row"><strong>Context:</strong> <span class="task-context-text">${escapeHtml(task.topic?.context || "—")}</span></div>
      <div class="task-meta-row"><strong>Infobox Color:</strong> <span class="color-swatch" style="background:${task.infoboxColor || "#ccc"}"></span> ${task.infoboxColor || "—"}</div>
      <div class="task-meta-row"><strong>Ecosystem:</strong> ${task.ecosystemStats ? `${task.ecosystemStats.totalArticles} articles, ${task.ecosystemStats.brokenLinks} broken, ${task.ecosystemStats.orphans} orphans` : "—"}</div>
    `;
  } catch (e) {
    alert("Error fetching preview: " + e.message);
  } finally {
    btn.disabled = false;
    btn.textContent = "Full Creative Brief";
  }
}
