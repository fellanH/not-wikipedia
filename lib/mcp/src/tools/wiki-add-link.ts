...
function addToSeeAlso(
  ...
  return {
    html:
      h.slice(0, ulEnd) +
      `\n            <li><a href="${target}">${display(target)}</a></li>` +
      h.slice(ulEnd),
    added: true,
  };
}
...
function addInlineLink(
  ...
  if (section) {
    const secRegex = new RegExp(
      `(<h2 id="${section}"[^>]*>[\\s\\S]*?)(<h2 |<\\/div>\\s*<\\/body>)`,
      "i",
    );
    const m = html.match(secRegex);
    if (m && regex.test(m[1]))
      return {
        html: html.replace(m[1], m[1].replace(regex, link)),
        added: true,
      };
...
export const tool: ToolModule = {
  definition: {
    name: "wiki_add_link",
    description:
      "Add cross-reference links between Not-Wikipedia articles. Supports see_also, inline, or both link types, with optional bidirectional linking.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        link_type: {
          type: "string",
          enum: ["see_also", "inline", "both"],
          description: "Link type (default: see_also)",
        },
        bidirectional: {
          type: "boolean",
          description: "Also add reverse link",
        },
        anchor_text: {
          type: "string",
          description: "Text to convert to inline link",
        },
        in_section: {
          type: "string",
          description: "Limit inline search to section",
        },
      },
  ...
  handler: async (args) => {
    try {
      ...
      let srcHtml: string;
      try {
        srcHtml = await fs.readFile(srcPath, "utf-8");
      } catch {
        return {
          content: [{ type: "text", text: `Error: '${srcFile}' not found` }],
          isError: true,
        };
      }
      ...
      try {
        tgtHtml = await fs.readFile(tgtPath, "utf-8");
      } catch {
      ...
      if (linkType === "see_also" || linkType === "both") {
        ...
        results.push(
          r.added
            ? `Added '${tgtFile}' to See Also in '${srcFile}'`
            : `Link already exists in See Also`,
        );
      }
      ...
      if ((linkType === "inline" || linkType === "both") && anchor) {
        ...
        results.push(
          r.added
            ? `Added inline link for '${anchor}'`
            : `Could not find '${anchor}' to link`,
        );
      }
      ...
      if (bidir && tgtHtml) {
        if (linkType === "see_also" || linkType === "both") {
          ...
          if (r.added) {
            tgtHtml = r.html;
            results.push(`Added bidirectional link to '${srcFile}'`);
          }
        }
