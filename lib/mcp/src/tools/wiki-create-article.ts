...
function extractSections(
  ...
  for (const line of content.split("\n")) {
    ...
    if (h2Match)
      sections.push({
        id: h2Match[1].trim().replace(/\s+/g, "_"),
        title: h2Match[1].trim(),
        level: 2,
      });
    else if (h3Match)
      sections.push({
        id: h3Match[1].trim().replace(/\s+/g, "_"),
...
function contentToHtml(content: string): string {
  ...
  html = html.replace(
    /^### (.+)$/gm,
    (_, t) => `<h3 id="${t.trim().replace(/\s+/g, "_")}">${t.trim()}</h3>`,
  );
  html = html.replace(
    /^## (.+)$/gm,
    (_, t) =>
      `<h2 id="${t.trim().replace(/\s+/g, "_")}">${t.trim()}<span class="mw-editsection">[<a href="?action=edit">edit</a>]</span></h2>`,
  );
...
function generateArticleHtml(input: ArticleInput): string {
  ...
  const infoboxHtml =
    input.infobox_fields && Object.keys(input.infobox_fields).length
      ? `\n        <table class="infobox">\n            <tr><td colspan="2" class="infobox-title">${input.title}</td></tr>\n            ${Object.entries(
          input.infobox_fields,
        )
  ...
  const seeAlsoHtml = input.see_also?.length
    ? input.see_also
        .map(
          (l) =>
            `<li><a href="${l.endsWith(".html") ? l : l + ".html"}">${l.replace(/-/g, " ").replace(".html", "")}</a></li>`,
        )
        .join("\n            ")
    : "<li><i>No related articles yet</i></li>";

  const categoriesHtml = input.categories?.length
    ? input.categories.map((c) => `<a href="#">${c}</a>`).join(" | ")
    : "<i>Uncategorized</i>";
...
export const tool: ToolModule = {
  definition: {
    ...
    description:
      "Create a new Not-Wikipedia article with proper HTML structure. Takes title and markdown content, generates complete HTML with infobox, TOC, and references.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        content: {
          type: "string",
          description:
            "Body content using markdown (## h2, ### h3, **bold**, *italic*, [link](url), - lists)",
        },
        topic: {
          type: "string",
          description: "Seed phrase to guide article topic",
        },
        infobox_color: {
          type: "string",
          description: "Hex color for infobox (e.g., '#7b9e89')",
        },
        infobox_fields: {
          type: "object",
          description: "Key-value pairs for infobox",
        },
        categories: {
          type: "array",
          items: { type: "string" },
        ...
        see_also: {
          type: "array",
          items: { type: "string" },
          description: "Related article filenames",
        },
  ...
  handler: async (args) => {
    try {
      ...
      if (!title || !content)
        return {
          content: [
            { type: "text", text: "Error: title and content required" },
          ],
          isError: true,
        };
      ...
      const input: ArticleInput = {
        ...
        infobox_fields: args.infobox_fields as
      ...
      try {
        ...
        return {
          content: [
            { type: "text", text: `Error: '${filename}' already exists` },
          ],
          isError: true,
        };
      } catch {
      ...
      const outlinks =
        (content.match(/\]\([^)]+\.html\)/g) || []).length +
        (input.see_also?.length || 0);
      ...
      try {
        ...
        throw new Error(
          `Article creation failed during DB registration: ${dbErr instanceof Error ? dbErr.message : dbErr}`,
        );
      }
