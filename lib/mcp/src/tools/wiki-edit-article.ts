...
function findSectionEnd(
  ...
  const end =
    nextH2 !== -1 && (contentEnd === -1 || nextH2 < contentEnd)
      ? start + match[0].length + nextH2
      : contentEnd !== -1
        ? start + match[0].length + contentEnd
        : html.length;
  return { start, end };
}
...
function addSection(
  ...
  if (after) {
    const pos = findSectionEnd(html, after);
    if (pos) return html.slice(0, pos.end) + section + html.slice(pos.end);
  }
  const m = html.match(/<h2 id="See_also"/i);
  return m?.index !== undefined
    ? html.slice(0, m.index) + section + html.slice(m.index)
    : html;
}
...
function updateSection(
  ...
  return (
    html.slice(0, pos.start + h2End.index + 5) +
    `\n        <p>${content}</p>\n` +
    html.slice(pos.end)
  );
}
...
function updateInfobox(html: string, field: string, value: string): string {
  ...
  const regex = new RegExp(
    `<th scope="row">${field}</th>\\s*<td>[^<]*</td>`,
    "i",
  );
  if (regex.test(html))
    return html.replace(
      regex,
      `<th scope="row">${field}</th><td>${value}</td>`,
    );
...
function addCategory(html: string, cat: string): string {
  ...
  if (html.includes("<i>Uncategorized</i>"))
    return html.replace("<i>Uncategorized</i>", `<a href="#">${cat}</a>`);
  const pos = m.index + m[0].length;
  const linkEnd = html.slice(pos).indexOf("</a>");
  return linkEnd !== -1
    ? html.slice(0, pos + linkEnd + 4) +
        ` | <a href="#">${cat}</a>` +
        html.slice(pos + linkEnd + 4)
    : html;
}
...
function applyEdit(html: string, op: EditOperation): string {
  switch (op.type) {
    case "add_section":
      return op.section_title && op.content
        ? addSection(html, op.section_title, op.content, op.after_section)
        : html;
    case "update_section":
      return op.section_id && op.content
        ? updateSection(html, op.section_id, op.content)
        : html;
    case "append_see_also":
      return op.link ? appendSeeAlso(html, op.link) : html;
    case "update_infobox":
      return op.infobox_field && op.infobox_value
        ? updateInfobox(html, op.infobox_field, op.infobox_value)
        : html;
    case "add_category":
      return op.category ? addCategory(html, op.category) : html;
    case "set_warning":
      return op.warning ? setWarning(html, op.warning) : html;
    case "append_content":
      return op.content ? appendContent(html, op.content) : html;
    default:
      return html;
  }
}
...
export const tool: ToolModule = {
  definition: {
    ...
    description:
      "Edit an existing Not-Wikipedia article. Operations: add_section, update_section, append_see_also, update_infobox, add_category, set_warning, append_content.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        operations: {
          ...
          items: {
            ...
            properties: {
              ...
              infobox_field: { type: "string" },
              infobox_value: { type: "string" },
  ...
  handler: async (args) => {
    try {
      ...
      if (!filename || !ops?.length)
        return {
          content: [
            { type: "text", text: "Error: filename and operations required" },
          ],
          isError: true,
        };
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: `Error: '${filename}' not found` }],
          isError: true,
        };
      }
      ...
      for (const op of ops) {
        const before = html;
        html = applyEdit(html, op);
        if (html !== before) applied.push(op.type);
      }
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { success: true, filename, operations_applied: applied },
              null,
              2,
            ),
          },
