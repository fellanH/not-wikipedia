...
async function parseArticle(
  ...
  const warnM = html.match(
    /<div class="ambox[^"]*">[^<]*(?:<strong>Warning:<\/strong>)?\s*([^<]+)/i,
  );
  if (warnM) meta.warning = warnM[1].trim();

  const infoM = html.match(/<table class="infobox">([\s\S]*?)<\/table>/i);
  if (infoM) {
    const titleM2 = infoM[1].match(
      /<td[^>]*class="infobox-title"[^>]*>([^<]+)<\/td>/i,
    );
    const colorM = html.match(
      /\.infobox-title\s*\{\s*background-color:\s*([^;]+)/i,
    );
    const fields: Array<{ label: string; value: string }> = [];
    const fieldRe =
      /<tr>\s*<th scope="row">([^<]+)<\/th>\s*<td>([\s\S]*?)<\/td>\s*<\/tr>/gi;
    let fm;
    while ((fm = fieldRe.exec(infoM[1])))
      fields.push({ label: fm[1].trim(), value: strip(fm[2]).trim() });
    meta.infobox = {
      title: titleM2?.[1]?.trim() || meta.title,
      fields,
      color: colorM?.[1]?.trim(),
    };
  }
  ...
  while ((sm = secRe.exec(html))) {
    ...
    const preview = strip(
      nextH > 0 ? after.slice(0, nextH) : after.slice(0, 500),
    ).slice(0, 200);
    meta.sections.push({
      id,
      title,
  ...
  if (seeAlsoM) {
    ...
    while ((lm = linkRe.exec(seeAlsoM[1]))) {
      if (!lm[1].startsWith("/wiki/") && !lm[1].startsWith("http"))
        meta.see_also.push(lm[1].replace(".html", ""));
    }
  }
  ...
  while ((alm = allLinkRe.exec(html))) {
    ...
    if (href.includes("action=edit") || href === "#" || seen.has(href))
      continue;
    seen.add(href);
    ...
    if (isInt) {
      let broken = false;
      try {
        await fs.access(path.join(WIKI_DIR, href));
      } catch {
        broken = true;
      }
...
export const tool: ToolModule = {
  definition: {
    ...
    description:
      "Parse a Not-Wikipedia article and return structured metadata: title, sections, links, infobox, categories, word count.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        include_content_preview: {
          type: "boolean",
          description: "Include section content previews (default: true)",
        },
        include_links: {
          type: "boolean",
          description: "Include detailed link analysis (default: true)",
        },
      },
      required: ["filename"],
    },
  ...
  handler: async (args) => {
    try {
      ...
      let html: string;
      try {
        html = await fs.readFile(path.join(WIKI_DIR, filename), "utf-8");
      } catch {
        return {
